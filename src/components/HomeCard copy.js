...
const HomeCard = ({onPress, videoInfo, theme}) => {
  const puplishDate = videoInfo?.snippet.publishedAt.split('');
  const stringPuplishDate = puplishDate?.slice(11, 16).join('');
  // console.log(puplishDate.slice(11, 16).join(''));
        ...
        <View className="gap-2 flex-row items-center">
          ...
          <View>
            ...
            <View className="flex-row gap-2">
              ...
              <Text className="text-stone-500">{stringPuplishDate}</Text>
            </View>
          </View>
        </View>
        <MyIcon
